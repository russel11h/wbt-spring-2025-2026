...
/* ============== Admin Dashboard: Manages Blood Donation Staff ============== */
function adminCtrl($conn) {
    ...
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($name === '' || $contact === '' || $username === '' || $password === '') {
            ...
            $error = 'Failed to add donor staff.';
        }
    }
    ...
    if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($name === '' || $contact === '' || $username === '') {
            $error = 'No field can be empty. All staff fields are required.';
            $editing = [
                'id' => $id,
                'name' => $name,
                'contact' => $contact,
            ...
            $error = 'That username is used by another staff member.';
            $editing = [
                'id' => $id,
                'name' => $name,
                'contact' => $contact,
...
/* ============== Staff Dashboard: Manages Blood Stock ============== */
function librarianCtrl($conn) {
    $action = $_GET['action'] ?? 'list';
    $error = '';
    $editing = null;

    /* --- Add Blood Stock (POST) --- */
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title === '' || $author === '' || $quantity === '' || $price === '') {
            ...
            $error = 'Failed to add blood stock record.';
        }
    }

    /* --- Update Blood Stock (POST) --- */
    if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $id       = intval($_GET['id'] ?? 0);
        $title    = trim($_POST['title'] ?? '');
        $author   = trim($_POST['author'] ?? '');
        ...
        if ($title === '' || $author === '' || $quantity === '' || $price === '') {
            $error = 'No field can be empty. All blood stock fields are required.';
            $editing = [
                'id' => $id,
                'title' => $title,
                'author' => $author,
            ...
            $error = 'Blood stock update failed.';
            $editing = [
                'id' => $id,
                'title' => $title,
                'author' => $author,
    ...
    /* --- Show Edit Blood Stock Form (GET) --- */
    if ($action === 'edit' && !$editing) {
        $id = intval($_GET['id'] ?? 0);
        $editing = getBook($conn, $id);
    }

    /* --- Delete Blood Stock (GET) --- */
    if ($action === 'delete') {
        $id = intval($_GET['id'] ?? 0);
