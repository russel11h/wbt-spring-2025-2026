...
function categoryCtrl($conn)
{
    ...
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($name === '' || !in_array($type, ['liquid', 'solid'])) {
            ...
            $upload = uploadCategoryImage('category_image');

            if (!$upload['ok']) {
                $error = $upload['message'];
            } else {
                addCategory($conn, $name, $type, $upload['path']);
    ...
    if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $name = trim($_POST['name'] ?? '');
        $type = $_POST['category_type'] ?? '';
        $oldCategory = getCategory($conn, $id);
        $oldImage = $oldCategory['image_path'] ?? 'asset/medicine-default.png';
        ...
        if ($name === '' || !in_array($type, ['liquid', 'solid'])) {
            ...
            $upload = uploadCategoryImage('category_image', $oldImage);
            ...
            if (!$upload['ok']) {
                ...
                updateCategory($conn, $id, $name, $type, $upload['path']);
    ...
    if ($action === 'edit') {
        $editing = getCategory($conn, intval($_GET['id'] ?? 0));
    }

    if ($action === 'delete') {
        $ok = deleteCategory($conn, intval($_GET['id'] ?? 0));

        header('Location: index.php?page=categories&msg=' . ($ok ? 'deleted' : 'blocked'));
        exit;
    }
...
function uploadCategoryImage($fieldName, $oldImage = '')
{
    if (
        ...
        return [
            'ok'      => true,
            'path'    => $oldImage ?: 'asset/medicine-default.png',
            'message' => ''
        ];
    }
    ...
    if (($_FILES[$fieldName]['size'] ?? 0) > 2 * 1024 * 1024) {
        return [
            'ok'      => false,
            'path'    => $oldImage,
            'message' => 'Category image must be less than 2 MB.'
        ];
    }
    ...
    if (!move_uploaded_file($_FILES[$fieldName]['tmp_name'], $newPath)) {
        return [
            'ok'      => false,
            'path'    => $oldImage,
            'message' => 'Could not save category image.'
        ];
    }

    if (
        $oldImage !== '' &&
        $oldImage !== 'asset/medicine-default.png' &&
...
function uploadMedicineImage($fieldName, $oldImage = '')
{
    ...
    if ($_FILES[$fieldName]['error'] !== UPLOAD_ERR_OK) {
        return [
            'ok'      => false,
            'path'    => $oldImage,
            'message' => 'Medicine image upload failed.'
        ];
    }
    ...
    if (($_FILES[$fieldName]['size'] ?? 0) > 2 * 1024 * 1024) {
        return [
            'ok'      => false,
            'path'    => $oldImage,
            'message' => 'Medicine image must be less than 2 MB.'
        ];
    }
    ...
    if (!move_uploaded_file($_FILES[$fieldName]['tmp_name'], $newPath)) {
        return [
            'ok'      => false,
            'path'    => $oldImage,
            'message' => 'Could not save medicine image.'
        ];
    }
...
function medicineCtrl($conn)
{
    ...
    if ($action === 'edit') {
        $editing = getMedicine($conn, intval($_GET['id'] ?? 0));
    }

    if ($action === 'delete') {
        $ok = deleteMedicine($conn, intval($_GET['id'] ?? 0));

        header('Location: index.php?page=medicines&msg=' . ($ok ? 'deleted' : 'blocked'));
        exit;
    }
...
function attachOrderDetails($conn, $orders)
{
    foreach ($orders as $key => $order) {
        ...
        $orders[$key]['payment'] = getPaymentByOrder($conn, $order['id']) ?: [
            'payment_method' => '',
            'transaction_id' => ''
        ];
    }
