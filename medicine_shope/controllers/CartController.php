...
function cartAddAjax($conn)
{
    ...
    if (!$medicine || $qty <= 0 || $qty > intval($medicine['availability'])) {
        ...
        echo json_encode([
            'ok'      => false,
            'message' => 'Invalid quantity or medicine stock.'
        ]);
    ...
    echo json_encode([
        'ok'    => true,
        'count' => cartCount($conn, $_SESSION['user']['id'])
    ]);
...
function cartRemoveAjax($conn)
{
    ...
    removeCartItem($conn, $_SESSION['user']['id'], $cartId);
...
function myOrdersCtrl($conn)
{
    ...
    $orders = attachCustomerOrderDetails(
        $conn,
        getUserOrders($conn, $_SESSION['user']['id'])
    );
